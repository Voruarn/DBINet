...
class DFM(nn.Module):
    def __init__(self, cur_in_ch, sup_in_ch, out_ch, mid_ch=64):
        ...
        self.cur_cv1 = ConvModule(cur_in_ch, mid_ch)      
        self.cur_cv2 = ConvModule(mid_ch, mid_ch)      

        self.sup_cv1 = ConvModule(sup_in_ch, mid_ch)
        self.sup_cv2 = ConvModule(mid_ch, mid_ch)
        self.fuse1 = ConvModule(mid_ch, mid_ch)
        self.fuse2 = ConvModule(mid_ch, out_ch)

    def forward(self, x_cur, x_sup):
        x_cur=self.cur_cv1(x_cur)
        x_sup=self.sup_cv1(x_sup)

        cur_fuse=self.cur_cv2(x_cur+x_sup)
        sup_fuse=self.sup_cv2(x_cur+x_sup)

        x=cur_fuse+sup_fuse
        x=x_cur+self.fuse1(x)
        x=self.fuse2(x)
        return x
